...
IND_NUMBER = 200   # Starting number of individuals in the population   
IND_SIZE = 3        # Number of gases in the mixture
ITER_NUM = 500   # Number of generations
...
print("Number of gases to be used in optimisation: " + str(len(data_selection)))
...
# Show 3D scatter plot of clustered gases
...
fig = px.scatter_3d(data_scaled, x='GWP', y='E/Ncrit', z='Boiling point', color=cluster.labels_, title=str(params_select[2:]), 
                    ...
                    symbol_sequence=symbols, hover_name='gas', labels={'GWP': 'GWP (normalised)', 'E/Ncrit': '(E/N)crit (normalised)', 'Boiling point': 'Boiling point (normalised)'})
...
def main(f):
    ...
    for gen_num in range(ITER_NUM):
        ...
        # Add data to the log
        f.write("Iteration: " + str(gen_num+1) + ' Gas occurrences in pop: ' + str(gas_popularity) + '\n') 
        ...
        for gas in gases_list:
            f.write('Gas occurrences in selection:' + gas.name + ' / ' + str(gas.fitness) + '#')
        f.write('\n')
        ...
        if len(sub_pop) > 0:
            if (((gen_num+1) % 100 == 0) or gen_num == 0):
        ...
        for i in range(display_size):
            #print(str(i+1) + ". " + mixture_id(pop[i]) + " " + str(pop[i].fitness.values))
        ...
        while len(new_gen) < pop_size:
            ...
            if mate:                    
                ...
                new_gen.append(parents[0])
                new_gen.append(parents[1]) 

            # Removing repeating mixtures
            check_list = mixtures_are_unique(new_gen)

            not_unique_index = []
            for index in range(len(new_gen)):
                current_mix_id = mixture_id(new_gen[index])
                mixture_duplicate = current_mix_id in check_list
                #if not compounds_are_unique(new_gen[index]): # or mixture_duplicate:
                if mixture_duplicate:
                    check_list.remove(current_mix_id)              
                    not_unique_index.append(index)
            if len(not_unique_index) > 0:
                for index in sorted(not_unique_index, reverse = True):
                    del new_gen[index]
        ...
        # Reset the compound popularity values
        reset_gases()
...
f = open('logs/evolution_log.txt', "a")  
